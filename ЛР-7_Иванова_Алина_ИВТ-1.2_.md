...
fn main() {
    ...
    println!("Updated First: {}", pair.get_first());
    println!("Updated Second: {}", pair.get_second());
}
```
...
fn main() {
    ...
    let task1 = Task::new(1, String::from("Write code"));
    let task2 = Task::new(2, String::from("Test code"));
...
```rust
trait Unique {
    fn is_unique<T: Eq>(&self, value: &T, list: &[T]) -> bool;
}

struct UniqueChecker;

impl Unique for UniqueChecker {
    fn is_unique<T: Eq>(&self, value: &T, list: &[T]) -> bool {
        list.iter().filter(|&x| x == value).count() == 1
    }
}

fn find_unique_elements<T: Eq + Clone>(vec: Vec<T>) -> Vec<T> {
    let checker = UniqueChecker;
    let mut result = Vec::new();

    for item in vec {
        if !result.contains(&item) && checker.is_unique(&item, &result) {
...
fn main() {
    ...
    println!("Unique elements: {:?}", unique_numbers);
}
```
...
## Задание 5: Напишите функцию `sum_collection`, которая принимает коллекцию элементов типа `T` и возвращает их сумму. Ограничьте тип `T` требованием наличия метода `addition()` в трейте `Addable`.
